...
//     logic signed [63:0] mix_res[1:0];
//     logic signed [63:0] mix_res_reg[1:0];  // ADDED: pipeline register
//     logic signed [63:0] makeup_scaled[1:0];
...
//             // CHANGED: Apply makeup gain BEFORE final saturation
//             // mix_res is 32-bit containing 16-bit audio
//             // Multiply by makeup_gain (128 = unity), then divide by 128
//             makeup_scaled[i] = ($signed(mix_res_reg[i]) * $signed({1'b0, fx_makeup_gain})) >>> 7;
//             mixed_signal[i] = makeup_scaled[i];
...
//                 x_cb_tmp_reg[i] <= '0;
//                 distorted_signal_reg[i] <= '0;
//                 mix_res_reg[i] <= '0;  // ADDED: initialize new register
//             end
//         end else if (sample_en) begin
//             for (int i = 0; i < 2; i++) begin
//                 audio_out[i] <= sat16(mixed_signal[i]);
...
//                 x_sq_reg[i] <= x_sq[i];
//                 distorted_signal_reg[i] <= distorted_signal[i];
//                 mix_res_reg[i] <= mix_res[i];  // ADDED: pipeline mix result
//             end
//         end
//     end
...
module fx_distortion #(
    ...
    logic signed [31:0] mix_res[1:0];        // CHANGED: 32-bit instead of 64-bit
    logic signed [31:0] mix_res_reg[1:0];    // ADDED: pipeline register
    logic signed [31:0] makeup_scaled[1:0];  // CHANGED: 32-bit instead of 64-bit
